...
async def analyse_student(responses):
    formatted_responses = format_student_responses(responses)
    system_prompt = STUDENT_GAP_ANALYSIS_PROMPT.format(subject="english", language="English")
    reply = await provider.generate(formatted_responses, system_prompt)
    return reply


async def analyse_class(responses):
    formatted_responses = format_class_responses(responses)
    system_prompt = CLASS_GAP_ANALYSIS_PROMPT.format(subject="english", language="English")
    reply = await provider.generate(formatted_responses, system_prompt)
    return reply
